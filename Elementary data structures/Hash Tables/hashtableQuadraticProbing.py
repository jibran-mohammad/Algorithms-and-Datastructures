"""
Implementation of hash table using quadratic probing in which h(k,i) =(h(k) + c1i +c2*i^2). On average the 
time complexity of insertion, deletion and search is going to be O(1). c1 is taken as 0 and c2 is taken as
1 
"""
...
class HashTable:
    ...
    def hashfunction(self, element: int, i: int)-> int:
        ...
        """
        return (element + (self.c1*i) + (self.c2 * (i**2))) % self.length

    def insert(self, element: int)-> None:
        """
        The insert function is going to probe all the slot sequences given by quadratic probing, if he finds the 
    ...
    def search(self, element: int) -> object:
        """
        search function similar to insert is going to probe all the sequences given by quadratic probing
